...
import autoTable from 'jspdf-autotable' // Import şekli düzeltildi
...
export default function StokYonetimi() {
  ...
  const [stokDurumFiltresi, setStokDurumFiltresi] = useState('HEPSI') // HEPSI, KRITIK, VAR, YOK
...
const handleStokKaydet = async (e: React.FormEvent) => {
  ...
  // Veriyi temizle (ilişkili objeleri çıkart)
...
const stokSil = async (id: string, ad: string) => {
  const result = await Swal.fire({
    ...
    borderRadius: '2rem',
    customClass: {
      popup: 'rounded-[2.5rem] font-bold'
    }
  });
    ...
    <div className="p-6 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto space-y-4">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setDuzenlemeId(null); setIsStokModalOpen(true); }} className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase shadow-xl hover:bg-indigo-600">+ Yeni Ürün</button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex justify-center items-center z-50">
          <div className="bg-white p-10 rounded-[3rem] w-full max-w-md">
            ...
            <div className="flex gap-2 mb-6">
              <input className="flex-1 p-4 bg-slate-50 rounded-2xl font-bold" value={yeniBirimAdi} onChange={e => setYeniBirimAdi(e.target.value)} placeholder="Yeni Birim (Örn: Adet, Koli)..." />
              <button onClick={handleBirimEkle} className="bg-slate-900 text-white px-6 rounded-2xl font-bold">+</button>
            </div>
            ...
            <button onClick={() => setIsBirimModalOpen(false)} className="w-full mt-6 text-[10px] font-black text-slate-400 uppercase">Kapat</button>
          </div>
        </div>
      )}
      ...
      {isStokModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex justify-center items-center z-50 p-4">
          <div className="bg-white p-5 rounded-[3.5rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <h2 className="text-3xl font-black mb-8 uppercase italic tracking-tighter">{duzenlemeId ? 'Kartı Güncelle' : 'Yeni Stok Kartı'}</h2>
            <form onSubmit={handleStokKaydet} className="grid grid-cols-2 gap-5">
              <input className="col-span-2 p-4 bg-slate-50 rounded-2xl font-bold border-none ring-1 ring-slate-100" placeholder="Ürün Adı" value={form.urun_adi} onChange={e => setForm({...form, urun_adi: e.target.value})} required />
              <select className="p-4 bg-slate-50 rounded-2xl font-bold text-xs" value={form.kategori_id} onChange={e => setForm({...form, kategori_id: e.target.value})}>
                <option value="">Kategori Seç...</option>
                {kategoriler.map(k => <option key={k.id} value={k.id}>{k.ad}</option>)}
              </select>
              <select className="p-4 bg-slate-50 rounded-2xl font-bold text-xs" value={form.birim_id} onChange={e => setForm({...form, birim_id: e.target.value})}>
                <option value="">Birim Seç...</option>
                {birimler.map(b => <option key={b.id} value={b.id}>{b.birim_adi}</option>)}
              </select>
            <select className="p-4 bg-slate-50 rounded-2xl font-bold text-xs" value={form.stok_tur_id} onChange={e => setForm({...form, stok_tur_id: e.target.value})}>
              <option value="">Stok Türü Seç...</option>
              {stok_turleri.map(t => <option key={t.id} value={t.id}>{t.tur_adi}</option>)}
            </select>
              <input className="p-4 bg-slate-50 rounded-2xl font-bold" placeholder="Barkod" value={form.barkod} onChange={e => setForm({...form, barkod: e.target.value})} />
              <input className="p-4 bg-slate-50 rounded-2xl font-bold" placeholder="Mevcut Stok" type="number" value={form.mevcut_stok} onChange={e => setForm({...form, mevcut_stok: Number(e.target.value)})} />
              <input className="p-4 bg-slate-50 rounded-2xl font-bold" placeholder="Alış Fiyatı" type="number" value={form.alis_fiyati} onChange={e => setForm({...form, alis_fiyati: Number(e.target.value)})} />
              <input className="p-4 bg-slate-50 rounded-2xl font-bold text-indigo-600" placeholder="Satış Fiyatı" type="number" value={form.satis_fiyati} onChange={e => setForm({...form, satis_fiyati: Number(e.target.value)})} />
              <input className="p-4 bg-slate-50 rounded-2xl font-bold" placeholder="KDV %" type="number" value={form.kdv_orani} onChange={e => setForm({...form, kdv_orani: Number(e.target.value)})} />
              <input className="p-4 bg-slate-50 rounded-2xl font-bold" placeholder="Kritik Stok Seviyesi" type="number" value={form.min_stok} onChange={e => setForm({...form, min_stok: Number(e.target.value)})} />
              <div className="col-span-2 p-4 bg-indigo-50 rounded-2xl flex items-center gap-3">
                <input type="checkbox" checked={form.satis_ekraninda_goster} onChange={e => setForm({...form, satis_ekraninda_goster: e.target.checked})} />
                <span className="text-xs font-black uppercase text-indigo-900">POS Satış Ekranında Göster</span>
              </div>
              <button type="submit" className="col-span-2 py-6 bg-slate-900 text-white rounded-[2rem] font-black uppercase text-sm shadow-xl hover:bg-indigo-600 transition-all">Kaydet</button>
           
            <button onClick={() => setIsStokModalOpen(false)} className="w-full mt-6 text-[10px] font-black text-slate-400 uppercase">Kapat</button>
